...
//        index = locate(index, key);
        Random i = getHashGenerator(key);
        //int index = i.nextInt();
        int index = locate(i, key);
...
//        index = locate(index, key);
    ...
    {
        boolean found = false;
        int i = index.nextInt(hashTable.length);
        
        while ( !found && (hashTable[i] != null) )
        {
            if ( hashTable[i].isIn() &&
                key.equals(hashTable[i].getKey()) )
                    found = true; // key found
            else // follow probe sequence
                i = index.nextInt(hashTable.length); // Linear probing
    ...
    public V add(K key, V value) {
        ...
        if ((key == null) || (value == null)) {
            ...
            Random i = getHashGenerator(key);
            //int index = abs(i.nextInt());
            int index = probe(i, key);
    ...
    private int probe(Random index, K key) {
        ...
        if (found || (removedStateIndex == -1) )
            return i; // Index of either key or null
        else
